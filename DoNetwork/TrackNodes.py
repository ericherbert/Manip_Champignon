...
def init():
    ...
    :param str img_path: absolute path of the image file on which to draw the 
        nodes, should be the image corresponding to the last graph in PNG format
    :param int size: size of the circle to draw for each node, in pixel
    """
    
    # General parameters
    vect_path = '/home/hyphes/LAURA/Test_0910/vect' # graphs directory
    csv_path = '/home/hyphes/LAURA/Test_0910/track/nodes_for_tracking.csv' # csv file 
    dest_path = '/home/hyphes/LAURA/Test_0910/track/' # output directory
    verbose = True
    ...
    img_path = '/home/hyphes/LAURA/stacks_tif/1705_binarizedMovie_6.png' # image file on which to draw
...
def matchAndInsert(df, graphs, verbose, forced_matching, log_path, log_txt):
    ...
    for row in tqdm(df.itertuples(), total=length, unit='row', mininterval=5,
        ...
        if len(nodes) == 1:
            graphs[frame].nodes[nodes[0]]['tag'] = getattr(row, 'particle')
            ...
            if forced_matching:
                ...
                for i, node in enumerate(nodes):
                    graphs[frame].nodes[nodes[i]]['tag'] = getattr(row, 
                                                                   'particle')
            else:
                txt = ('Non matching coordinates at frame {}, row index {}: '
