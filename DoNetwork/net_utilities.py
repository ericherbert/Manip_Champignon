...
def drawGraphTriangulation(G, triangles, image_name, dest, distance_map,
    ...
    # nx.draw_networkx_edges(G, pos=pos, width=widths, edgecolor='DimGray', alpha=0.4)
    nx.draw_networkx_edges(G, pos=pos, width=widths, alpha=0.4)
...
def _drawGraph(G, verbose, n_size, height):
    ...
    if verbose:
        print("\t from _drawGraph: drawing took %1.2f sec"%(time.perf_counter()-start))
