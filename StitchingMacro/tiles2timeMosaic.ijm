...
#@ Integer(label="first time point to analyse",value=1,persist=true) minTp
...
#@ String (label="tiles layout order", value="Down & Left", choices={"Down & Left", "Up & Right"}, persist=true) layoutOrder
...
run("Image Sequence... ", "format=TIFF save="+myHyperDir+" name = /hyperbrutes_t001_z001.tif");
...
function flatFieldCorrect(timeStr, tempHyperTpName, tempFFCOutFolder, tempFCCNameOut){
	...
	run("Image Sequence... ", "format=TIFF  save="+tempFFCOutFolder+" start=1 digits=3 name="+tempFCCNameOut );
}	
